class DostsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @dost.save
        format.html { redirect_to @dost, notice: "Dost was successfully created." }
  ...
  def update
    respond_to do |format|
      if @dost.update(dost_params)
        format.html { redirect_to @dost, notice: "Dost was successfully updated." }
  ...
  def correct_user
   ...
   redirect_to dosts_path, notice: "Not authorized to edit this dost" if @dost.nil?
  end
  ...
  private
    ...
    def dost_params
      params.require(:dost).permit(:first_name, :ladt_name, :email, :phone, :twitter, :user_id)
    end
end
